...
epsilon ~ B e r n (p)
$
случайная $epsilon$ распределена по Бернулли
...
Из распределения Бернулли складываются другие распределения
...
Пусть произведено $n$ незвисимых одинаковых испытаний Бернулли
...
$ F_(xi) = limits(sum)_(k < x) C_n^k p^k q^(n - k) $
...
=== Распределение Пуассона (Предельные распределения).

$
epsilon ~ "Pois" (lambda); space lambda = n p
$
