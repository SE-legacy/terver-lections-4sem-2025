...
Мы гооворили о классах подмножеств.

$Omega$ --- это множество элементарных исходов.

/ Кольцо над множеством $Omega$: --- назовем класс подмножесв $Re$ такой, что:
...
1. $omega in cal(F)$
2. Если $A in cal(F)$, то $overline(A) in cal(F)$
3. ${A_i}_(i = 1)^infinity in cal(F) => limits(union)_(i = 1)^infinity A_i in cal(F)$

/ $sigma$-алгебра событий: множество всех возможных подмножеств множества $Omega$, т.е. множество всех мыслимых эксперементов.


$ cal(F) = {Omega, emptyset, {omega_1} {omega_2},...,{omega_6}, {omega_1, omega_2}... {omega_5, omega_6}, {omega_3, omega_4, omega_5 }"все тройки"\ ", четверки и пятерки"} $

*Опр:*
Пусть $Omega$ --- некоторое множество, а $K$ --- класс подмножеств $Omega$ 

Минимальной $sigma$-алгеброй над классом $A$ называется $sigma$-алгебра $sigma(K): K in sigma(K), sigma(K) subset cal(F) = sigma(Omega)$
...
// TODO: Нужно у пацанов потом взять дописать
